var IK = (function () {
  ...
  var robot;
  var kinematics;
  var marker, shadow_marker;

  var controlPointMarkers;
  var target_object, table;
  ...
  module.init = function (options) {
    ...
    var geometry = new THREE.BoxGeometry(0.3, 0.3, 1.0);
    shadow_marker = new THREE.Mesh( geometry, material );
    viewer.scene.add(shadow_marker);
//    shadow_marker.up = new THREE.Vector3(0,0,1);
    ...
    viewer.scene.add(table);
    table.position.set(6.0, 0.0, 2.25);
  ...
  function getControlPoints(obj) {
      ...
      var x_offset = new THREE.Vector3(2, 0, 0);
      var y_offset = new THREE.Vector3(0, 2, 0);
      var points = [
        pos.clone(),
  ...
  function getControlPointsRot(obj) {
      var pos = obj.getWorldPosition().clone();
      var x_offset = new THREE.Vector3(0, 0, 2);
      var y_offset = new THREE.Vector3(0, 2, 0);
      var points = [
        pos.clone(),
        pos.clone().add(x_offset.applyQuaternion(obj.quaternion)),
  ...
  function reorientGripper(marker,shadow_marker,obj) {
    // Set orientation of gripper to point towards center of obj
    shadow_marker.position.copy(marker.position);
    shadow_marker.lookAt(obj.position);
  }

  function displayControlPoints(points) {
    for (var i = 0; i < points.length; i++) {
      controlPointMarkers[i].position.copy(points[i]);
    }
  }
  ...
  function solveIK() {
      if (use2d) {
          ...
          reorientGripper(marker,shadow_marker,target_object);
          var controlPoints = getControlPointsRot(shadow_marker);
